...
def roll(key, prob):
    ...
    if key[0] == 0:         #Lose
        print("lose")
    elif key[1] == 0:         #no dice remain
        #sVec = [sVec[0]+prob*key, sVec[1]+prob]
        print("win!")
    else:
        #newprob = prob*listOfProbDicts[1][key]
        #newdice = key[1]
        #newsoft = soft + key[0]
        print("continue")
        #manager(newdice, gene, sVec, newprob, newsoft)
"""
...
def decide(gene, sVec, nDice):
    if s>= gene[0]:
        sVec = [sVec[0], sVec[1]]
    else:
        for key in listOfProbDicts[1]:
            manager(key, listOfProbDicts[1][key])
